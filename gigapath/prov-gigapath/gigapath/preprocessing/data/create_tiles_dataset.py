...
def generate_tiles(slide_image: np.ndarray, tile_size: int, foreground_threshold: float,
    ...
    print(f"Tiled {slide_image.shape} to {image_tiles.shape}")
...
def process_slide(sample: Dict["SlideKey", Any], level: int, margin: int, tile_size: int,
    ...
    if is_already_processed(output_tiles_dir):
        ...
        print(f">>> Copying {slide_image_path} to {tmp_slide_image_path}")
        ...
        # print(f"Saving thumbnail {thumbnail_dir / (slide_image_path.name + '_roi.png')}, shape {slide_image.shape}")
