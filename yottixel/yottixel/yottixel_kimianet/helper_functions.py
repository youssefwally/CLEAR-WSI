...
def leave_one_out_test(
    ...
    if len(atlas_bobs) < top_n:
        top_n = len(atlas_bobs)
    ...
    total_metrics = {"percision": 0.0, "recall": 0.0, "NDCG": 0.0, "f1": 0.0, "correct_majority_vote": 0.0}
    for out_index in tqdm(
        ...
        metrics = get_topk_stats(_test_case, _atlas_bobs, _atlas_labels, true, top_n)
        ...
        total_metrics["correct_majority_vote"] = total_metrics["correct_majority_vote"] + metrics["correct_majority_vote"]
    ...
    total_metrics["percision"] = total_metrics["percision"] / number_of_cases
    total_metrics["recall"] = total_metrics["recall"] / number_of_cases
    total_metrics["NDCG"] = total_metrics["NDCG"] / number_of_cases
    total_metrics["f1"] = total_metrics["f1"] / number_of_cases
    total_metrics["correct_majority_vote"] = total_metrics["correct_majority_vote"] / number_of_cases
        
                           
    return y_true, y_pred, sorted_labels, sorted_distances, total_metrics

def get_topk_stats(new_bob, atlas_bobs, atlas_labels, true_label, k):
    #search_image, sorted_all_targets
    # Compute the distances
    distances = np.array([new_bob.distance(bob) for bob in atlas_bobs])
    ...
    sorted_all_targets = np.array(atlas_labels)[similar_indices]
    ...
    for i in range(k_search):
        if (sorted_all_targets[i] == true_label):
            relavent += 1
    ...
    print(majority_vote, flush=True)
